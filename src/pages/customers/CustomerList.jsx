    ...
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs md:text-sm text-gray-500">{customers.length} total</p>
        </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-100">
                  {['Name', 'Email', 'Phone', 'Invoiced', 'Outstanding', ''].map(h => (
                    <th key={h}
              ...
              <tbody className="divide-y divide-gray-50">
                {filtered.map(row => (
                  <tr key={row._id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3.5 text-sm text-gray-600">{row.email || '—'}</td>
                    <td className="px-4 py-3.5 text-sm text-gray-600">{row.phone || '—'}</td>
                    <td className="px-4 py-3.5 text-sm font-medium">
                      ${(row.totalInvoiced || 0).toFixed(2)}
                    </td>
                    <td className="px-4 py-3.5 text-sm font-semibold">
                      <span className={row.totalOutstanding > 0 ? 'text-red-600' : 'text-green-600'}>
                ...
                <div className="grid grid-cols-2 gap-2 pt-3 border-t border-gray-50">
                  ...
                  <div className="bg-gray-50 rounded-lg p-2 text-center">
                    ...
                    <p className={`text-sm font-bold ${
                      row.totalOutstanding > 0 ? 'text-red-600' : 'text-green-600'
                    }`}>
                      ${(row.totalOutstanding || 0).toFixed(2)}
                    </p>
                  </div>
      ...
      {/* ── Add Customer Modal ─────────────────────────────────────────── */}
      <Modal open={modal} onClose={() => { setModal(false); setForm(EMPTY); }}
        title="Add Customer">
        <div className="space-y-4">
          ...
          <div className="flex flex-col sm:flex-row justify-end gap-2 pt-2 border-t border-gray-100">
            <Button variant="secondary"
              onClick={() => { setModal(false); setForm(EMPTY); }}
