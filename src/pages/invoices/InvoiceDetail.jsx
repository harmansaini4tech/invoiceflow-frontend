...
import {
  ...
  MoreVertical, X, Link2, DollarSign, // ✅ added DollarSign
} from "lucide-react";
        ...
        <div className="p-6 space-y-4">
          <div className="bg-gray-50 rounded-xl p-3 flex justify-between">
            ...
            <span className="font-bold text-red-600">
              {invoice.currency} {(invoice.balanceDue || invoice.total || 0).toFixed(2)}
            </span>
          </div>
          <div>
...
export default function InvoiceDetail() {
  ...
  useEffect(() => {
    if (invoice) {
      api.get(`/payments/${invoice._id}/history`)
  ...
  const handleWhatsApp = async () => {
    ...
    try {
      ...
      const message = `Hello ${customerName}! 👋\n\n*Invoice from ${companyName}*\n━━━━━━━━━━━━━━━━━━\n📄 *Invoice #:* ${invoice.invoiceNumber}\n📅 *Issue Date:* ${issueDate}\n⏰ *Due Date:* ${dueDate}\n💰 *Amount Due:* ${invoice.currency} ${invoice.total?.toFixed(2)}\n📌 *Status:* ${invoice.status?.toUpperCase()}\n━━━━━━━━━━━━━━━━━━\n\n${invoice.lineItems?.map(item => `• ${item.description} × ${item.quantity} = ${invoice.currency} ${item.amount?.toFixed(2)}`).join("\n")}\n\n━━━━━━━━━━━━━━━━━━\n${invoice.subtotal !== invoice.total ? `Subtotal: ${invoice.currency} ${invoice.subtotal?.toFixed(2)}\n` : ""}${invoice.taxAmount > 0 ? `Tax (${invoice.taxRate}%): ${invoice.currency} ${invoice.taxAmount?.toFixed(2)}\n` : ""}${invoice.discountAmount > 0 ? `Discount: -${invoice.currency} ${invoice.discountAmount?.toFixed(2)}\n` : ""}*Total Due: ${invoice.currency} ${invoice.total?.toFixed(2)}*\n\n${invoice.notes ? `📝 *Note:* ${invoice.notes}\n` : ""}Please find the attached PDF invoice and make payment by the due date.\n\nThank you for your business! 🙏\n_Powered by InvoiceFlow_`;
      ...
      setTimeout(() => {
        ...
        if (customerPhone) {
          ...
          toast("💡 Tip: Add customer phone number to send directly!", { duration: 4000, icon: "📱" });
        }
      }, 800);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-4 md:space-y-6">
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <h1 className="text-lg md:text-2xl font-bold truncate">{invoice.invoiceNumber}</h1>
        ...
        <div className="hidden md:flex items-center gap-2 flex-shrink-0">
          <button
            ...
            className="inline-flex items-center gap-2 font-semibold py-2.5 px-4 rounded-lg transition-all text-sm bg-gray-100 hover:bg-gray-200 text-gray-700">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 font-semibold py-2.5 px-4 rounded-lg transition-all text-sm disabled:opacity-50 bg-green-500 hover:bg-green-600 text-white shadow-sm">
          ...
          {/* ✅ Record Payment button — desktop */}
          {invoice.status !== "paid" && (
            <button
              onClick={() => setShowRecordPayment(true)}
              className="inline-flex items-center gap-2 font-semibold py-2.5 px-4 rounded-lg transition-all text-sm bg-blue-500 hover:bg-blue-600 text-white shadow-sm">
      ...
      {showMobileActions && (
        <>
          <div className="md:hidden fixed inset-0 bg-black/40 z-40" onClick={() => setShowMobileActions(false)} />
          <div className="md:hidden fixed bottom-0 inset-x-0 bg-white rounded-t-2xl z-50 p-4 shadow-2xl">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowMobileActions(false)} className="p-1.5 hover:bg-gray-100 rounded-lg">
                <X className="w-5 h-5 text-gray-500" />
              </button>
            </div>
            <div className="space-y-2">
              <button
                ...
                <div>
                  <p className="font-semibold text-sm text-gray-900">Copy Public Link</p>
                  <p className="text-xs text-gray-500">Share invoice with customer</p>
                </div>
              </button>
              ...
              <button
                ...
                <div>
                  <p className="font-semibold text-sm text-gray-900">Share via WhatsApp</p>
                  <p className="text-xs text-gray-500">Send invoice details on WhatsApp</p>
                </div>
              </button>
              ...
              {invoice.status !== "paid" && (
                <button
                  ...
                  <div>
                    <p className="font-semibold text-sm text-gray-900">Send via Email</p>
                    <p className="text-xs text-gray-500">Email invoice to customer</p>
                  </div>
                </button>
              )}
              ...
              {invoice.status !== "paid" && (
                <button
                  onClick={() => { setShowMobileActions(false); setShowRecordPayment(true); }}
                  className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-blue-50 transition-colors text-left">
                  <div className="w-10 h-10 bg-blue-100 rounded-xl flex items-center justify-center flex-shrink-0">
                    <DollarSign className="w-5 h-5 text-blue-600" />
                  </div>
                  <div>
                    <p className="font-semibold text-sm text-gray-900">Record Payment</p>
                    <p className="text-xs text-gray-500">Record partial or full payment</p>
                  </div>
                </button>
              )}
              ...
              {invoice.status !== "paid" && (
                <button
                  ...
                  <div>
                    <p className="font-semibold text-sm text-gray-900">Mark as Paid</p>
                    <p className="text-xs text-gray-500">Record full payment received</p>
                  </div>
                </button>
              )}
              ...
              <button
                ...
                <div>
                  <p className="font-semibold text-sm text-red-600">Delete Invoice</p>
      ...
      <div className="card p-4 md:p-6 space-y-5 md:space-y-6">
        <div className="grid grid-cols-2 gap-4 md:gap-8">
          <div>
            <p className="text-xs font-semibold text-red-600 uppercase tracking-wider mb-1 md:mb-2">From</p>
            <p className="font-bold text-gray-900 text-sm md:text-lg truncate">{invoice.company?.name}</p>
            <p className="text-xs md:text-sm text-gray-500 truncate">{invoice.company?.email}</p>
            {invoice.company?.phone && <p className="text-xs md:text-sm text-gray-500">{invoice.company.phone}</p>}
          </div>
          <div>
            <p className="text-xs font-semibold text-red-600 uppercase tracking-wider mb-1 md:mb-2">Bill To</p>
            <p className="font-bold text-gray-900 text-sm md:text-lg truncate">{invoice.customer?.name}</p>
            <p className="text-xs md:text-sm text-gray-500 truncate">{invoice.customer?.email}</p>
            {invoice.customer?.phone && <p className="text-xs md:text-sm text-gray-500">{invoice.customer.phone}</p>}
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 md:gap-4 bg-gray-50 rounded-xl p-3 md:p-4">
          <div>
            ...
            <p className="font-semibold text-gray-900 text-xs md:text-sm truncate">{invoice.invoiceNumber}</p>
          </div>
          <div>
        ...
        <div className="hidden md:block rounded-xl overflow-hidden border border-gray-100">
          ...
          {invoice.lineItems?.map((item, i) => (
            <div key={i} className={`grid grid-cols-12 px-4 py-3 border-t border-gray-50 ${i % 2 === 0 ? "bg-white" : "bg-gray-50/50"}`}>
              ...
              <div className="col-span-2 text-sm text-right text-gray-600">{invoice.currency} {item.rate?.toFixed(2)}</div>
              <div className="col-span-2 text-sm font-semibold text-right">{invoice.currency} {item.amount?.toFixed(2)}</div>
            </div>
          ))}
        </div>
          ...
          {invoice.lineItems?.map((item, i) => (
            <div key={i} className="bg-gray-50 rounded-xl p-3">
              <div className="flex justify-between items-start mb-1">
                ...
                <p className="text-sm font-bold text-gray-900 flex-shrink-0">{invoice.currency} {item.amount?.toFixed(2)}</p>
              </div>
              <p className="text-xs text-gray-500">{item.quantity} × {invoice.currency} {item.rate?.toFixed(2)}</p>
            </div>
          ))}
        </div>
          ...
          <div className="w-full md:w-72 space-y-2 bg-gray-50 md:bg-transparent rounded-xl p-3 md:p-0">
            ...
            {invoice.discountAmount > 0 && (
              <div className="flex justify-between text-sm">
                ...
                <span className="text-green-600 font-medium">-{invoice.currency} {invoice.discountAmount?.toFixed(2)}</span>
              </div>
            )}
            {invoice.taxAmount > 0 && (
              <div className="flex justify-between text-sm">
            ...
            {invoice.shippingCharge > 0 && (
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium">{invoice.currency} {invoice.shippingCharge?.toFixed(2)}</span>
              </div>
            )}
      ...
      {!invoice.customer?.phone && (
        <div className="flex items-start gap-3 bg-green-50 border border-green-200 rounded-xl p-3 md:p-4">
          <WhatsAppIcon />
          <div>
            <p className="text-sm font-semibold text-green-800">Add phone number for direct WhatsApp</p>
            <p className="text-xs text-green-600 mt-0.5">
              Go to <strong>Customers</strong> → edit <strong>{invoice.customer?.name}</strong> → add phone with country code (e.g. +91XXXXXXXXXX)
            </p>
          </div>
        </div>
      )}
